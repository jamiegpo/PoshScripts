...
$sNewOUParentContainer = "OU=NGD-Workstations,DC=contoso,DC=com"									# OU under which new OUs wil be created
...
# Check Groups
if ((Check-GroupExists ($sGroupName + "-Full Control") + Check-GroupExists ($sGroupName + "-Add Remove") + Check-GroupExists ($sGroupName + "-Add Remove OU")) -ne 0)
{
...
{
	$sidFullControlGroup = (Create-Group ($sGroupName + "-Full Control") "Members have Full Control of computer objects in the relevant OU" $constCreateGroupOU)
	$sidAddRemoveGroup = (Create-Group ($sGroupName + "-Add Remove") "Members can add and remove computer objects in the relevant OU" $constCreateGroupOU)
	$sidAddRemoveOUGroup = (Create-Group ($sGroupName + "-Add Remove OU") "Members can add and remove OU objects under the relevant OU" $constCreateGroupOU)
}
...
try
{
	...
	$acl.AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $sidFullControlGroup, “GenericAll”, “Allow”,  “Descendents”, $guidmap["computer"]))
	$acl.AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $sidFullControlGroup,"CreateChild,DeleteChild","Allow",$guidmap["computer"],"All"))
	
	Write-Host "Setting Add Remove permisssions on ACL" -ForegroundColor Green
	$acl.AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $sidAddRemoveGroup,"CreateChild,DeleteChild","Allow",$guidmap["computer"],"All"))
	
	Write-Host "Setting Add Remove OU permisssions on ACL" -ForegroundColor Green
	$acl.AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $sidAddRemoveOUGroup,"CreateChild,DeleteChild","Allow",$guidmap["organizationalunit"],"All"))
